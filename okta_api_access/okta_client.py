...
def hello():
    if (request.args.get('action') and request.args.get('action') == 'login'):
        ...
        return get_users()
    elif (request.args.get('action') and request.args.get('action') == 'messages'):
        return get_messages()
...
def get_users():
    ...
    profile = api_request(client_config.api_url_base + '/api/v1/users?' + urllib.parse.urlencode(params), ssws=client_config.okta_ssws_token)
    the_response = str(profile)
